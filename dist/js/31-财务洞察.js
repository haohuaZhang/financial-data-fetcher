...
function buildFinanceAlerts(summary) {
  if (summary.fileCount === 0) return [{ level: 'success', text: '暂无数据，开始采集后将自动生成财务洞察' }];
  const alerts = [];
  for (const metric of summary.metrics) {
    ...
    if (metric.key === 'profit' && s.current < 0) alerts.push({ level: 'danger', text: `净利润为负：${formatFinanceNumber(s.current)}` });
    if (metric.key === 'assets' && summary.metrics.find(m => m.key === 'liabilities')?.snapshot) {
      ...
      if (Number.isFinite(liabilities) && Number.isFinite(s.current) && liabilities > s.current) alerts.push({ level: 'warn', text: '负债合计高于总资产，建议重点复核' });
    }
  }
  if (alerts.length === 0) alerts.push({ level: 'success', text: '暂无明显异常，整体数据稳定' });
...
function renderFinanceMetricTable(summary) {
  ...
  if (rows.length === 0) {
    body.innerHTML = '<tr><td colspan="6" class="finance-table-empty">暂无数据</td></tr>';
    return;
  }
